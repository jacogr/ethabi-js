...
function padU32(input) {
  ...
  if (bn.lessThan(0)) {
    bn = new BigNumber('ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff', 16).plus(bn).plus(1);
  }
...
function padFixedBytes(input) {
  ...
  if (isArray(input)) {
    sinput = input.map(function (code) {
...
function padString(input) {
  var encoded = utf8.encode(input).split('').map(function (char) {
...
var Mediate = function () {
  ...
  createClass(Mediate, [{
    ...
    key: 'init',
    value: function init(suffixOffset) {
      var _this = this;
      ...
      switch (this._type) {
        ...
        case 'fixedArray':
          return this._value.map(function (mediate, idx) {
            return mediate.init(Mediate.offsetFor(_this._value, idx)).toString(16);
          }).join('');
    ...
    value: function closing(offset) {
      ...
      switch (this._type) {
        ...
        case 'fixedArray':
          return this._value.map(function (mediate, idx) {
            return mediate.closing(Mediate.offsetFor(_this2._value, idx)).toString(16);
          }).join('');

        case 'array':
          var prefix = padU32(this._value.length);
          var inits = this._value.map(function (mediate, idx) {
            return mediate.init(offset + Mediate.offsetFor(_this2._value, idx) + 32).toString(16);
          }).join('');
          var closings = this._value.map(function (mediate, idx) {
            return mediate.closing(offset + Mediate.offsetFor(_this2._value, idx)).toString(16);
          }).join('');
    ...
    key: 'offsetFor',
    value: function offsetFor(mediates, position) {
    ...
    value: function validateType(type) {
      if (TYPES.filter(function (_type) {
        return type === _type;
      }).length) {
        return true;
      }
...
var Token = function () {
  ...
  createClass(Token, [{
    ...
    key: 'validateType',
    value: function validateType(type) {
      if (TYPES$1.filter(function (_type) {
        return type === _type;
      }).length) {
        return true;
      }
...
var Encoder = function () {
  ...
  createClass(Encoder, null, [{
    ...
    value: function encode(tokens) {
      ...
      var mediates = tokens.map(function (token) {
        return Encoder.encodeToken(token);
      });
      var inits = mediates.map(function (mediate, idx) {
        return mediate.init(Mediate.offsetFor(mediates, idx));
      }).join('');
      var closings = mediates.map(function (mediate, idx) {
    ...
    key: 'encodeToken',
    value: function encodeToken(token) {
...
var Param = function () {
  ...
  createClass(Param, [{
    ...
    key: 'toParams',
    value: function toParams(params) {
...
var Constructor = function () {
  ...
  createClass(Constructor, [{
    ...
    key: 'encodeCall',
    value: function encodeCall(tokens) {
...
function asI32(slice) {
  if (new BigNumber(slice.substr(0, 1), 16).toString(2)[0] === '1') {
    return new BigNumber(slice, 16).minus(new BigNumber('ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff', 16)).minus(1);
  }
...
var Decoder = function () {
  ...
  createClass(Decoder, null, [{
    ...
    key: 'peek',
    value: function peek(slices, position) {
    ...
    key: 'takeBytes',
    value: function takeBytes(slices, position, length) {
      ...
      var bytes = bytesStr.substr(0, length * 2).match(/.{1,2}/g).map(function (code) {
    ...
    key: 'decodeParam',
    value: function decodeParam(param, slices, offset) {
      ...
      switch (param.type) {
        ...
        case 'string':
          ...
          var str = taken.bytes.map(function (code) {
...
var EventParam = function () {
  ...
  createClass(EventParam, [{
    ...
    key: 'toEventParams',
    value: function toEventParams(params) {
      return params.map(function (param) {
        return new EventParam(param.name, param.type, param.indexed);
      });
    }
  }]);
  return EventParam;
...
function methodSignature(name, params) {
...
var Event = function () {
  ...
  createClass(Event, [{
    ...
    key: 'inputParamNames',
    value: function inputParamNames() {
      return this._inputs.map(function (input) {
        return input.name;
      });
    }
    ...
    key: 'indexedParams',
    value: function indexedParams(indexed) {
    ...
    key: 'decodeLog',
    value: function decodeLog(topics, data) {
      ...
      var topicTypes = topicParams.map(function (param) {
        return param.kind;
      });
      var flatTopics = topics.filter(function (topic, idx) {
        return idx >= toSkip;
      }).join('');
      ...
      var dataTypes = dataParams.map(function (param) {
        return param.kind;
      });
      ...
      var decodedParams = this.inputParamNames().map(function (name, idx) {
        return new DecodedLogParam(name, inputParamTypes[idx], namedTokens[name]);
      });
...
var Func = function () {
  ...
  createClass(Func, [{
    ...
    key: 'outputParamTypes',
    value: function outputParamTypes() {
    ...
    key: 'decodeOutput',
    value: function decodeOutput(data) {
...
module.exports = EthAbi;/* Mon Jul 18 08:01:27 UTC 2016 */
